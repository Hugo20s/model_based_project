...
etape_M <- function(data, K, tk){
  ...
  #   for (i in 1:N) {
  #     
  #     sig <- sig + tk[, k]*(data[i,] - moyenne[k,]) %*% t(data[i,] - moyenne[k,])
  #   }
  #   variance[,,k] <- sig / nk
  # }
  # 
...
main <- function(data, K, epislon, type_init = "kmeans", parameters = 0){
  ...
  while (i > 0) {
    ...
    if (abs(log_likelihood[i] - log_likelihood[i-1]) < epislon){
      ...
      return(list(log_likelihood = log_likelihood[-1], y_pred = y_pred, maxll = max(log_likelihood[-1]), parameters = parameters))
    }
...
# 
for (i in 1:100){
  res <- main(data, 4, epsilon, "kmeans")
  if (typeof(res) == "list"){
